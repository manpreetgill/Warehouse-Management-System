...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
router.route('/v1/processMaster/mobile/pickSublist/action/update-status/in-progress/')
        ...
        .post(function (req, res) {
            ...
            flowController.on('START', function () {
                ...
                pickSubListModel.findOne({'_id': pickSubListId, 'activeStatus': 1}, function (err, pickSubListRow) {
                    ...
                    if (err) {
                        ...
                        pickSubListModel.find({'pickListId': pickListId, 'resourceAssigned.deviceId': deviceId, 'activeStatus': 1}).select({'name': 1}).exec(function (err, pickSubListMongoIds) {

                            if (err) {

                                flowController.emit('ERROR', {message: "INTERNAL SERVER ERROR " + err, status: 'error', statusCode: '500'});
                            } else if (pickSubListMongoIds == null) {

                                flowController.emit('ERROR', {message: 'No line items not available.', status: 'error', statusCode: '304'});
                            } else {

                                var MongoIds = pickSubListMongoIds;

                                flowController.emit('1', MongoIds, pickListId);
                            }
                        });
                    }
                });
            ...
            flowController.on('1', function (MongoIds, pickListId) {
                ...
                pickSubListModel.findOne({'_id': pickSubListId, 'activeStatus': 1}, function (err, pickSubListRow) {
                    if (err) {
                        ...
                        flowController.emit('ERROR', {message: 'No line item details available in system', status: 'error', statusCode: '304'});
                    } else if (pickSubListRow.status < 11) {
                        ...
                        pickSubListModel.update(query, update, function (err) {
                            if (err) {
                                ...
                                flowController.emit('2', MongoIds, pickListId);
                            }
                        });
                    }
                });
            ...
            flowController.on('2', function (MongoIds, pickListId) {

                (consoleVar) ? console.log('2') : '';

                pickListModel.findOne({'_id': pickListId}, function (err, pickListRecord) {
                    if (err) {
                    ...
                    } else if ('timeStarted' in pickListRecord) {

                        pickSubListModel.find({pickListId: pickListId}).sort({'status': 1}).limit(1).exec(function (err, pickSublistRow) {
                            if (err) {

                                flowController.emit('ERROR', {message: "INTERNAL SERVER ERROR " + err, status: 'error', statusCode: '500'});
                            } else {

                                pickListRecord.status = pickSublistRow[0].status;
                                pickListRecord.save(function (err) {
                                    if (err) {
                        ...
                        pickListRecord.status = 25;
                        pickListRecord.timeStarted = timeInInteger;
                        pickListRecord.startedBy = startedBy;
            ...
            flowController.on('PICK-ALERT', function (MongoIds) {
                ...
                pickSubListModel.findOne({'_id': pickSubListId, 'activeStatus': 1}, function (err, pickSubListRow) {
                    ...
                    if (err) {
                        ...
                        itemMasterModel.findOne({'itemCode': pickSubListRow.itemCode}, function (err, itemMasterRow) {
            ...
            flowController.on('DONE', function (result) {
                ...
                currentActivityStatusFunction('PICK', pickSubListId, 'PICK - In progress');
                res.json(result);
            });
...
//--------------------------------------------------------------------------------------------------------------------------------
router.route('/v1/processMaster/mobile/pickSublist/action/update-status/pending-for-drop/')
            ...
            pickSubListModel.findOne({'_id': pickSubListId, 'activeStatus': 1}, function (err, pickSubListRow) {
                if (err) {
                    ...
                    flowController.on('START', function () {
                        ...
                        var query = {'_id': pickSubListId};
                        var update = {'$set': {'status': 27, 'pickedQuantity': pickSubListRow.requiredQuantity}};
                        ...
                        pickSubListModel.update(query, update, function (err) {
                            if (err) {
                                ...
                                if (itemType == 'ITEMCODE')
                                    ...
                                    flowController.emit('1', pickSubListRow.itemStoreId);
                            }
                        });
                    });
                    ...
                    flowController.on('1', function (itemStoreArray) {
                        ...
                        var pickLocationId = pickSubListRow.pickLocationId;
                                    ...
                                    if (err) {
                                        ...
                                        if (functionAreaRow.name == 'REPROCESS' || functionAreaRow.name == 'DISPATCH') {
                                            ...
                                            if (in_Array) {
                                                ...
                                                currentLength = locationStoreRow.availableCapacity + pickSubListRow.requiredQuantity;
                    ...
                    flowController.on('4', function (itemStoreArray) {
                        ...
                        var query = {'_id': pickSubListId, 'activeStatus': 1};
                    ...
                    flowController.on('END', function (result) {
                        currentActivityStatusFunction('PICK', pickSubListId, 'PICK - Pending for drop');
                        res.json(result);
                    });
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
router.route('/v1/processMaster/mobile/pickSubList/action/update-status/done/')
            ...
            pickSubListModel.findOne({'_id': pickSubListId, 'activeStatus': 1}, function (err, pickSubListRow) {
                if (err) {
                    ...
                    flowController.on('UPDATE_LOCATION_STORE', function () {
                        ...
                        var itemStoreId = pickSubListRow.itemStoreId;

                        var dropLocationId = pickSubListRow.dropLocationId;
                            ...
                            if (err) {
                                ...
                                functionAreaModel.findOne({'_id': dropLocationFunction, 'activeStatus': 1}, function (err, functionAreaRow) {
                                    ...
                                    if (err) {
                                        ...
                                        if (functionAreaRow.name == 'REPROCESS' || functionAreaRow.name == 'DISPATCH') {
                                        ...
                                        } else if (pickSubListRow.itemType == 'PALLET' && pickSubListRow.palletType == 'O') {
                                            ...
                                            if (in_Array) {
                                                ...
                                                var capacityToBeReduced = pickSubListRow.requiredQuantity;
                    ...
                    flowController.on('UPDATE_ITEMSTORE', function (functionAreaRow) {
                        ...
                        async.eachSeries(itemStoreId, function (element, callback) {
                            ...
                            if (dropLocationFunctionArea === 'REPROCESS') {
                                ...
                                update = {
                                    '$set': {'locationStoreId': pickSubListRow.dropLocationId},
                                    '$unset': {'customPalletNumber': 1}
                                };
                            } else if (dropLocationFunctionArea === 'DISPATCH') {
                                itemStoreIdForAlert = element;
                                update = {'$set': {'locationStoreId': pickSubListRow.dropLocationId, 'activeStatus': 4}};
                            } else {

                                update = {'$set': {'locationStoreId': pickSubListRow.dropLocationId}};
                            }
                    ...
                    flowController.on('END', function (result) {
                        ...
                        currentActivityStatusFunction('PICK', pickSubListId, 'PICK - Done');
                        setTimeout(function () {
                            exportPickSubListDataToCybernatic(baseUrl, pickSubListId);
                        }, 1000);
                        res.json(result);
                    });
...
//--------------------------------------------------------------------------------------------------------------------------------
router.route('/v1/processMaster/mobile/pickSublist/action/update-status/skipped/')
        ...
        .patch(function (req, res) {
                                ...
                                pickListModel.findOne({'_id': pickSubListRow.pickListId, 'activeStatus': 1}, function (err, pickListRow) {
                                    if (err) {
                                        ...
                                        var dataObject = {
                                            ...
                                            textName: "Picklist moved to skipped with itemCode " + pickSubListRow.itemCode,
                                            module: "PICKSUBLIST",
                                            name: "",
                                            id: pickSubListRow._id
                                        };
                                        alertService.createAlert(dataObject);
                                        flowController.emit('LOGS');
                                        flowController.emit('END', {message: "Status updated into the system!", status: 'success', statusCode: '200'});
            ...
            // END
            flowController.on('END', function (result) {
                currentActivityStatusFunction('PICK', pickSubListId, 'PICK - Skipped');
                res.json(result);
            });
...
router.route('/v1/processMaster/mobile/pickSubList/action/update-partial/pick-location/location-scan/')
        ...
        .patch(function (req, res) {
            ...
            flowController.on('2', function (pickSubListRow) {
                ...
                pickListModel.findOne({'_id': pickListId, 'activeStatus': 1}, function (err, pickListRecord) {
                    if (err) {
...
router.route('/v1/processMaster/mobile/pickSubList/action/update-partial/pick-location/item-scan/')
                                    ...
                                    if (err) {
                                        ...
                                        if (functionAreaRow.name == 'REPROCESS' || functionAreaRow.name == 'DISPATCH') {
                                            ...
                                            if (in_Array) {
                                                ...
                                                var currentLength = locationStoreRow.availableCapacity + pickSubListRow.requiredQuantity;
...
router.route('/v1/processMaster/mobile/pickSubList/action/update-partial/drop-location/location-scan/')
        ...
        .patch(function (req, res) {
            ...
            var pickSubListId = req.body.pickSubListId;
            var baseUrl = req.body.baseUrl.trim();
            ...
            locationStoreModel.findOne({'customerAddress': dropLocation, 'availability': 'A', 'activeStatus': 1}).exec(function (err, locationStoreRow) {
                if (err) {
                    ...
                    flowController.on('VALIDATE_DROP_LOCATION', function () {
                        ...
                        pickSubListModel.findOne({'_id': pickSubListId, 'activeStatus': 1}, function (err, pickSubListRow) {
                            if (err) {
                                ...
                                if (pickSubListRow.dropLocationAddress == dropLocation) {

                                    flowController.emit('FINAL_UPDATE');
                                } else {

                                    flowController.emit('VALIDATE_0_HOLDINGTYPE', pickSubListRow);
                                }
                            }
                        });
                    });
                    ...
                    flowController.on('RULE_ENGINE', function (pickSubListRow) {
                    ...
                    flowController.on('VALIDATE_2_SCHEDULED_DROP_PICK', function (pickSublistRow, availableCapacity, putReservedCapacity) {
                        ...
                        pickSubListModel.find({'status': {"$lt": 31}, 'dropLocationId': dropLocationId, 'activeStatus': 1}, function (err, pickSubListRow2) {
                    ...
                    flowController.on('GET_ITEMMASTER', function (pickSubListRow) {
                        ...
                        itemMasterModel.findOne({'itemCode': itemCode, 'activeStatus': 1}, function (err, itemMasterRow) {
                            ...
                            if (err) {
                                ...
                                flowController.emit('GET_VALID_PICK_FUNCTIONS', itemMasterRow, pickSubListRow);
                            }
                        });
                    });
                    //
                    // Get valid pick process functions
                    flowController.on('GET_VALID_PICK_FUNCTIONS', function (itemMasterRow, pickSubListRow) {
                        ...
                        var query = {};
                        query.name = (pickSubListRow.itemType === 'ITEMCODE') ? {'$in': ['STORAGE']} : {'$in': ['REPROCESS', 'STORAGE']};
                            ...
                            if (err) {
                                ...
                                async.eachSeries(functionAreaRow, function (element, callback) {
                                    ...
                                    if (err) {
                                        ...
                                        flowController.emit('GET_VALID_DROP_FUNCTIONS', itemMasterRow, pickSubListRow, functionAreaArray);
                                    }
                                });
                            }
                        });
                    ...
                    flowController.on('GET_VALID_DROP_FUNCTIONS', function (itemMasterRow, pickSubListRow, pickFunctionArea) {
                            ...
                            if (err) {
                                ...
                                async.eachSeries(functionAreaMongoIdRow, function (element, callback) {
                                    ...
                                    if (err) {
                                        ...
                                        flowController.emit('VALIDATE_FUNCTIONS', itemMasterRow, pickSubListRow, pickFunctionArea, dropFunctionArea);
                                    }
                                });
                            }
                        });
                    ...
                    flowController.on('VALIDATE_FUNCTIONS', function (itemMasterRow, pickSubListRow, pickFunctionArea, dropFunctionArea) {
                        ...
                        if (dropFunctionArea.indexOf(dropLocationFunction) == -1) {
                            ...
                            if (pickFunctionArea.indexOf(dropLocationFunction) > -1) {
                                ...
                                flowController.emit('VALIDATE_ITEM_RESERVATION', itemMasterRow, pickSubListRow);
                            } else {

                                flowController.emit('UPDATE_DROP_LOCATION_TO_PICKSUBLIST', pickSubListRow);
                            }
                        }
                    });
                    //
                    //
                    flowController.on('VALIDATE_ITEM_RESERVATION', function (itemMasterRow, pickSubListRow) {
                        ...
                        if (locationStoreRow.isReservedForItem == 'YES') {
                            ...
                            if (locationStoreRow.reservedItemId.indexOf(itemMasterId) > -1) {
                                ...
                                if (locationStoreRow.reservedItemId.length > 1) {
                                    ...
                                    flowController.emit('VALIDATE_ELIGIBILITY', itemMasterRow, pickSubListRow);

                                } else {
                                    //
                                    flowController.emit('UPDATE_DROP_LOCATION_TO_PICKSUBLIST', itemMasterRow, pickSubListRow);
                                }
                            } else {
                                //
                    ...
                    flowController.on('VALIDATE_ELIGIBILITY', function (itemMasterRow, pickSubListRow) {
                    ...
                    flowController.on('UPDATE_DROP_LOCATION_TO_PICKSUBLIST', function (pickSubListRow) {
                            ...
                            if (err) {
                                ...
                                pickSubListModel.update(query, update, function (err) {
                                    if (err) {
                                        ...
                                        flowController.emit('UPDATE_TIMECOUNT', pickSubListRow);
                                    }
                                });
                            }
                        });
                    ...
                    //
                    // Update timecount of sublist
                    flowController.on('UPDATE_TIMECOUNT', function (pickSubListRow) {
                        ...
                        pickListModel.update(query, update, function (err) {
                            if (err)
                                ...
                                flowController.emit('UPDATE_LOCATION', pickSubListRow);
                        });
                    });
                    //
                    // Add items to new location
                    flowController.on('UPDATE_LOCATION', function (pickSubListRow) {
                            ...
                            locationStoreModel.update(query, update, function (err) {
                                ...
                                if (err) {
                                    ...
                                    if (dummyObject.length === itemStoreId.length) {
                                        ...
                                        flowController.emit('UPDATE_DROP_LOCATION_CAPACITY', pickSubListRow);
                                    }
                                }
                            });
                        });
                    ...
                    //
                    // Update available quantity at location store if it is user defined
                    flowController.on('UPDATE_DROP_LOCATION_CAPACITY', function (pickSubListRow) {
                                ...
                                functionAreaModel.findOne({'_id': dropLocationFunction, 'activeStatus': 1}, function (err, functionAreaRow) {
                                    ...
                                    if (err) {
                                        ...
                                        if (functionAreaRow.name == 'REPROCESS' || functionAreaRow.name == 'DISPATCH') {
                                            ...
                                            flowController.emit('UPDATE_ITEMSTORE', pickSubListRow, functionAreaRow);
                                            //} else if (pickSubListRow.itemType == 'PALLET' && pickSubListRow.palletType == 'O') {
                    ...
                    //
                    // update item store for in transit
                    flowController.on('UPDATE_ITEMSTORE', function (pickSubListRow, functionAreaRow) {
                        ...
                        async.eachSeries(itemStoreId, function (element, callback) {
                            ...
                            if (dropLocationFunctionArea === 'REPROCESS') {
                                ...
                                var update = {
                                    '$set': {'locationStoreId': pickSubListRow.dropLocationId, 'currentActivityStatus': 'PICK-COMPLETED'},
                                    '$unset': {'customPalletNumber': 1}
                                };
                            } else if (dropLocationFunctionArea === 'DISPATCH') {

                                var update = {'$set': {'locationStoreId': pickSubListRow.dropLocationId, 'currentActivityStatus': 'PICK-COMPLETED', 'activeStatus': 4}};
                            } else {

                                var update = {'$set': {'locationStoreId': pickSubListRow.dropLocationId, 'currentActivityStatus': 'PICK-COMPLETED'}};
                            }
                    ...
                    flowController.on('END', function (result) {
                        ...
                        currentActivityStatusFunction('PICK', pickSubListId, 'PICK - Done(Manual)');
                        res.json(result);
                    });
                    //
                    // ERROR
...
router.route('/v1/processMaster/mobile/pickSubList/action/update-pickSubList/manual-override-reason/')
...
//
//
router.route('/v1/processMaster/mobile/pickSubList/action/update-outer/drop-location/location-scan/')

        .patch(function (req, res) {

        });

function exportPickSubListDataToCybernatic(baseUrl, pickSubListId) {

    var requestifyUrl = baseUrl + '/v1/processMaster/web/pickList/action/create/manual-export/';

    data = {pickSubListId: pickSubListId};
    ...
    requestify.post(requestifyUrl, data).then(function (response) {
        ...
        if (result.status === 'success')
            console.log(pickSubListId + 'Pick export complete: ' + result.message);
...
//
var currentActivityStatusFunction = function (model, pickSubListId, status) {

    currentActiveStatusService.setCurrentActivityStatus(model, pickSubListId, status, function (err, records) {
        if (err) {
            console.log(err);
        } else {
