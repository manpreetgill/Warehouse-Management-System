...
//---------------------------------------------------------------------------------------------------------------------------
router.route('/v1/processMaster/web/pickList/configuration/create/manual-import/')
...
//                                                            }
            ...
            flowController.on('7', function (pickListId, allowedZoneArray, palletNumberArray, serialNumberArray, itemCodeArray) {

                (showConsole) ? console.log('7') : '';

                pickListModel.findOne({'_id': pickListId, 'activeStatus': 1}, function (err, pickListRow) {

                    pickListRow.referenceFile = fileName; //Pick_15028630872
                    pickListRow.mode = 'MANUAL';
                    pickListRow.save(function (err) {

                        flowController.emit('8', pickListId, allowedZoneArray, palletNumberArray, serialNumberArray, itemCodeArray);
                    });
                });
            });
            //
            // Pallet number based PickSublists
            flowController.on('8', function (pickListId, allowedZoneArray, palletNumberArray, serialNumberArray, itemCodeArray) {
                ...
                async.eachSeries(palletNumberArray, function (element, callback) {
                    ...
                    async.waterfall([
                        ...
                        function (waterfallcallback) {
                            ...
                            pickSubListModel.find({'status': {"$lt": 31}, 'itemType': 'PALLET', 'itemValue': palletNumber, 'activeStatus': 1}, function (err, pickSubListRow) {
                        ...
                        function (putReservedCapacity, waterfallcallback) {
                            ...
                            if (data.isDispatch == 'YES') {
                                ...
                                pickSubListModel.find({'status': {"$lt": 31}, 'dropLocationId': dropLocationId, 'activeStatus': 1}, function (err, pickSubListRow) {
                        ...
                        function (itemStoreRowObject, waterfallcallback) {
                            ...
                            newPickSubList.itemCode = data.itemMaster.itemCode;
                            newPickSubList.itemType = 'PALLET';
                            newPickSubList.itemValue = palletNumber;
                            (itemStoreRowObject.palletType === 'O') ? newPickSubList.customPalletNumber = element.customPalletNumber : '';
                            newPickSubList.pickActivity = 'FULL';
                            newPickSubList.palletType = itemStoreRowObject.palletType;
                            newPickSubList.palletSize = itemStoreRowObject.palletSize;
                            newPickSubList.hopperPriority = 2;
                            newPickSubList.itemStoreId = itemStoreRowObject.itemStoreId;
                            newPickSubList.serialNumberArray = itemStoreRowObject.serialNumberArray;
                            newPickSubList.itemDescription = data.itemMaster.itemDescription;
                            newPickSubList.requiredQuantity = 1;
                            newPickSubList.pickLocationId = data.pickLocation._id;
                            newPickSubList.pickLocationAddress = data.pickLocation.customerAddress;
                            newPickSubList.dropLocationId = data.dropLocation._id;
                            newPickSubList.dropLocationAddress = data.dropLocation.customerAddress;
                            newPickSubList.sequence = '';
                            newPickSubList.createdBy = createdBy;
                            newPickSubList.timeCreated = timeInInteger;

                            palletInsertionArray.push(newPickSubList);
                    ...
                    if (validationErrorArray.length > 0) {
                        ...
                        flowController.emit('ERROR', {message: 'Following error occurred while validating excel file.', validationErrors: validationErrorArray, status: 'error', statusCode: '304', tempId: pickListId});
                    } else {

                        flowController.emit('9', pickListId, allowedZoneArray, palletInsertionArray, serialNumberArray, itemCodeArray);
                    }
                });
            });
            //
            // Serial number based PickSublists
            flowController.on('9', function (pickListId, allowedZoneArray, palletInsertionArray, serialNumberArray, itemCodeArray) {
                ...
                async.eachSeries(serialNumberArray, function (element, callback) {
                    ...
                    async.waterfall([
                        ...
                        function (waterfallcallback) {
                            ...
                            pickSubListModel.find({'status': {"$lt": 31}, 'itemType': 'SERIALNUMBER', 'itemValue': serialNumber, 'activeStatus': 1}, function (err, pickSubListRow) {
                        ...
                        function (itemStoreRowObject, waterfallcallback) {
                            ...
                            newPickSubList.pickListId = pickListId;
                            newPickSubList.itemCode = data.itemMaster.itemCode;
                            newPickSubList.itemType = 'SERIALNUMBER';
                            newPickSubList.itemValue = serialNumber;
                            newPickSubList.pickActivity = 'FULL';
                            newPickSubList.hopperPriority = 2;
                            newPickSubList.itemStoreId = itemStoreRowObject.itemStoreId;
                            newPickSubList.serialNumberArray = itemStoreRowObject.serialNumberArray;
                            ...
                            serialInsertionArray.push(newPickSubList);
                    ...
                    if (validationErrorArray.length > 0) {
                        ...
                        flowController.emit('10', pickListId, allowedZoneArray, palletInsertionArray, serialInsertionArray, itemCodeArray);
                    }
                });
            });
            //
            // Item Code based PickSublists
            flowController.on('10', function (pickListId, allowedZoneArray, palletInsertionArray, serialInsertionArray, itemCodeArray) {
                ...
                async.eachSeries(itemCodeArray, function (element, callback) {
                    ...
                    async.waterfall([
                        ...
                        function (waterfallcallback) {
                            ...
                            async.eachSeries(data.itemStore, function (element, callback) {
                                ...
                                pickSubListModel.find({'itemStoreId': itemStoreId, 'status': {'$lt': 31}, 'activeStatus': 1}, function (err, pickSubListRow) {
                        ...
                        function (itemStoreRowObject, waterfallcallback) {
                            ...
                            newPickSubList.pickListId = pickListId;
                            newPickSubList.itemCode = data.itemMaster.itemCode;
                            newPickSubList.itemType = 'ITEMCODE';
                            newPickSubList.itemValue = itemCode;
                            newPickSubList.pickActivity = 'FULL';
                            //newPickSubList.palletType = itemStoreRowObject.palletType;
                            //newPickSubList.palletSize = itemStoreRowObject.palletSize;
                            newPickSubList.hopperPriority = 2;
                            ...
                            itemCodeInsertionArray.push(newPickSubList);
                    ...
                    if (validationErrorArray.length > 0) {
                        ...
                        flowController.emit('11', pickListId, palletInsertionArray, serialInsertionArray, itemCodeInsertionArray);
                    }
                });
            });
            //
            // End
            flowController.on('11', function (pickListId, palletInsertionArray, serialInsertionArray, itemCodeInsertionArray) {
                ...
                async.waterfall([
                    ...
                    function (waterfallcallback) {
                        ...
                        if (palletInsertionArray.length == 0) {
                            ...
                            async.eachSeries(palletInsertionArray, function (element, callback) {
                                ...
                                newPickSubList.pickListId = element.pickListId;
                                newPickSubList.itemCode = element.itemCode;
                                newPickSubList.itemType = element.itemType;
                                newPickSubList.itemValue = element.itemValue;
                                (element.palletType === 'O') ? newPickSubList.customPalletNumber = element.customPalletNumber : '';
                                newPickSubList.pickActivity = element.pickActivity;
                                newPickSubList.palletType = element.palletType;
                                newPickSubList.palletSize = element.palletSize;
                                newPickSubList.hopperPriority = element.hopperPriority;
                                newPickSubList.itemStoreId = element.itemStoreId;
                                newPickSubList.serialNumberArray = element.serialNumberArray;
                                newPickSubList.itemDescription = element.itemDescription;
                                newPickSubList.requiredQuantity = element.requiredQuantity;
                                newPickSubList.pickLocationId = element.pickLocationId;
                                newPickSubList.pickLocationAddress = element.pickLocationAddress;
                                newPickSubList.dropLocationId = element.dropLocationId;
                                newPickSubList.dropLocationAddress = element.dropLocationAddress;
                                newPickSubList.sequence = sequence;
                                newPickSubList.createdBy = element.createdBy;
                                newPickSubList.timeCreated = element.timeCreated;

                                newPickSubList.save(function (err, data) {
                                    if (err) {
                                        waterfallcallback('ERROR', err);
                                    } else {
                                        sequence++;

                                        currentActivityStatusFunction('PICK', data._id.toString(), 'PICK - Scheduled(Manual)');

                                        var query = {'_id': pickListId};
                                        var update = {'$addToSet': {'pickSubLists': data._id.toString()}};
                                        ...
                                        pickListModel.update(query, update, function (err) {
                                            ...
                                            if (err) {
                                                ...
                                                addProcessLogs(pickListId, createdBy, dataObject);
                                                setImmediate(callback);
                                            }
                                        });
                                    }
                    ...
                    if (err) {
                        ...
                        flowController.emit('12', pickListId, palletInsertionArray, serialInsertionArray, itemCodeInsertionArray);
                    }
                });
            });
            //
            // Update and set name to picklist
            flowController.on('12', function (pickListId, palletInsertionArray, serialInsertionArray, itemCodeInsertionArray) {
                ...
                pickListModel.findOne({'warehouseId': warehouseId, 'date': date}).sort({'name': -1}).exec(function (err, pickListRow) {
                    ...
                    if (err) {
                        ...
                        var name = (pickListRow == null) ? 'PIK' + moment(new Date()).format('DDMM') + '0001' : MagicIncrement.inc(pickListRow.name);
                        var sequence = (pickListRow == null) ? 1 : MagicIncrement.inc(pickListRow.sequence);
            ...
            flowController.on('ERROR', function (error) {
                ...
                if (error.hasOwnProperty('tempId')) {
                    ...
                    pickListModel.remove({_id: String(error.tempId)}, function (err) {
                        if (err)
                            console.log('Error');
                        else
...
//---------------------------------------------------------------------------------------------------------------------------
router.route('/v1/processMaster/web/pickList/action/create/manual-export/')
        ...
        .post(function (req, res) {
            ...
            flowController.on('START', function () {
                ...
                pickSubListModel.findOne({'_id': pickSubListId, 'activeStatus': 1}, function (err, pickSubListRow) {
                    if (err) {
                        ...
                        flowController.emit('0', pickSubListRow);
                    }
                });
            });

            // Get line item
            flowController.on('0', function (pickSubListRow) {
                ...
                pickListModel.findOne({'_id': pickSubListRow.pickListId, 'activeStatus': 1}, function (err, pickListRow) {
                    if (err) {
                        ...
                        flowController.emit('1', pickListRow, pickSubListRow);
                    }
                });
            });

            // Get latest file name
            flowController.on('1', function (pickListRow, pickSubListRow) {

                (consoleLog) ? console.log('1') : '';

                var rgx = new RegExp("^" + moment(new Date()).format('DDMMYYHHMMSSSSS'));

                var pickFileName = 'PIK' + moment(new Date()).format('DDMMYYHHMMSSSSS') + '_001';

                cyberneticFilesModel.findOne({'warehouseId': pickListRow.warehouseId, 'pickName': {$regex: rgx}}).sort({'pickName': -1}).exec(function (err, cyberneticFilesRow) {
                    if (err) {
                        ...
                        flowController.emit('1.1', pickListRow, pickSubListRow, fileName);
                    }
                });
            });

            // 
            flowController.on('1.1', function (pickListRow, pickSubListRow, fileName) {

                var cyberneticFile = new cyberneticFilesModel();

                cyberneticFile.warehouseId = pickListRow.warehouseId;
                ...
                cyberneticFile.save(function (err) {
                    if (err)
                        ...
                        flowController.emit('2', pickListRow, pickSubListRow, fileName);
                });
            });

            // Build the JSON
            flowController.on('2', function (pickListRow, pickSubListRow, fileName) {
                ...
                async.eachSeries(serialNumberArray, function (element, callback) {
                    ...
                    itemStoreModel.findOne({'itemSerialNumber': element, 'activeStatus': {'$in': [1, 4]}}, function (err, itemStoreRow) {
                        if (err) {
                            ...
                            data.BoxNo = element;
                            data.PalletNo = (pickSubListRow.itemType == 'PALLET') ? pickSubListRow.itemValue : '';
                            data.Rack = pickSubListRow.dropLocationAddress;
                    ...
                    if (err)
                        ...
                        flowController.emit('3', pickListRow, pickSubListRow, fileName, finalArray);
                });
            });

            // Generate file
            flowController.on('3', function (pickListRow, pickSubListRow, fileName, finalArray) {
            ...
            flowController.on('END', function (response) {
                ...
                console.log('FILE UPLOADED :' + pickSubListId);
                res.json(response);
            });
...
//
//    
var addProcessLogs = function (pickListId, createdBy, dataObject) {

    pickListModel.findOne({'_id': pickListId, 'activeStatus': 1}, function (err, pickListRow) {
        userModel.findOne({'_id': createdBy, 'activeStatus': 1}, function (err, userRow) {
            username = (userRow.username) ? userRow.username : '';

            fs.appendFile(pathPickSubList, '\n' + 'WEB' + ',' + 'CREATE' + ',' + 'PICKSUBLIST' + ',' + username + ',' + pickListRow.name + ',' + dataObject.itemCode + ',' + dataObject.itemType + ',' + dataObject.itemValue + ',' + dataObject.deviceId + ',' + dataObject.orderNumber + ',' + dataObject.pickLocationAddress + ',' + dataObject.dropLocationAddress + ',' + dataObject.timeInInteger + ',' + moment(new Date()).format('DD/MM/YY-HH:MM:SS'), function (err) {
                if (err) {
