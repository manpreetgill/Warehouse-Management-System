...
//
//
pickListService.updateStatusToInActivate = function (dataObject, servicecallback) {
    ...
    flowController.on('START', function () {
        ...
        function_pickListService.getPicklistDataByDate(pickListId, function (err, pickListRecord) {
            if (err) {

                flowController.emit('ERROR', err);
            } else {

                flowController.emit('1', pickListRecord);
            }
        });
    });
    ...
    flowController.on('1', function () {
        ...
        function_pickListService.getAllPicklistDataByActivatedSequence(function (err, pickListActivatedRecord) {
            if (err) {

                flowController.emit('ERROR', err);
            } else {

                flowController.emit('2', pickListActivatedRecord);
            }
        });
    });
    ...
    flowController.on('2', function (lastSequence) {
        ...
        function_pickSubListService.getPickSublistDataById(pickListId, function (err, pickSubListRecords) {
            if (err) {

                flowController.emit('ERROR', err);
            } else {

                flowController.emit('2.0', lastSequence, pickSubListRecords);
            }
        });
    });
    // Check if any of the Pick Sublist is OUTER & Check if all outer are picked or not.
    flowController.on('2.0', function (lastSequence, pickSubListRecords) {
        ...
        async.eachSeries(pickSubListRecords, function (element, callback) {
            ...
            if (element.itemType === 'PALLET') {
                ...
                if (palletType === 'O' && status <= 5) {
                    ...
                    function_itemStoreService.checkPickActivityByCustomPalletNumber(pickListId, customPalletNumber, function (err) {
                        if (err) {
    ...
    flowController.on('4', function (newSequence) {
        ...
        function_pickListService.updatePicklistStatusToActivated(pickListId, newSequence, function (err, response) {
            if (err) {
    ...
    flowController.on('5', function (newSequence) {
        ...
        function_pickSubListService.updatePickSublistStatusToActivated(pickListId, newSequence, function (err, response) {
            if (err) {
    ...
    flowController.on('LOGS', function () {
        ...
        function_pickSubListService.addTransactionalLogAfterActivationPicklist(warehouseId, pickListId, function (err, response) {
            if (err) {
...
//
//
pickListService.updateStatusToAssign = function (dataObject, servicecallback) {
    ...
    flowController.on('START', function () {
        ...
        function_pickListService.checkIfInitialLotAlreadyAssigned(deviceId, baseUrl, function (err, result) {
    ...
    flowController.on('1', function () {
        ...
        function_pickListService.getAllPicklistDataByActivatedSequenceAscending(function (err, picklistRecords) {
    ...
    flowController.on('2', function (picklistRecords) {
        ...
        async.eachSeries(picklistRecords, function (element, callbackDone) {
            ...
            function_pickSubListService.getAllActivatedPickSublistDataByPicklistId(element.pickListId, function (err, pickSubListRecords) {
                ...
                if (err) {
                    ...
                    async.eachSeries(pickSubListRecords, function (element2, callback2Done) {

                        var data = {
                            pickListId: element.pickListId,
                            pickSubListId: String(element2._id),
                            name: element.name,
                                ...
                                if (err) {
                                    ...
                                    if (pickLocationMHE.length > 0 && itemMHE.length > 0 && dropLocationMHE.length > 0) {
                                        ...
                                        if (commonArray.indexOf(singleMHU) > -1) {
                                            ...
                                            var data = {
                                                pickListId: element.pickListId,
                                                pickSubListId: element.pickSubListId,
                                                name: element.name,
                                                hopperSequence: element.hopperSequence,
                                                hopperPriority: element.hopperPriority,
    ...
    flowController.on('6.0', function (resultArray) {
        ...
        async.eachSeries(resultArray, function (element, callbackDone) {
            ...
            if (element.customPalletNumber == '') {
                ...
                var pickListId = element.pickListId;
                var customPalletNumber = element.customPalletNumber;

                function_pickSubListService.getAllPickSublistByCustomPalletNumber(pickListId, customPalletNumber, function (err, pickSubListRecords) {
                    if (err) {

                        callbackDone(err);
                    } else {

                        async.eachSeries(pickSubListRecords, function (element2, callbackDone2) {

                            var pickSubListId = String(element2._id);

                            var object = {};
                            object.pickListId = element.pickListId;
                            object.pickSubListId = pickSubListId;
    ...
    flowController.on('6', function (resultArray) {
        ...
        async.eachSeries(resultArray, function (element, callbackDone) {
            ...
            async.eachSeries(resultArray, function (element, callback2Done) {
                ...
                if (pickListId == element.pickListId) {

                    temp.push(element.pickSubListId);
                    setImmediate(callback2Done);
                } else {
                ...
                if (err) {
                    ...
                    if (picklistIdArray.indexOf(pickListId) == -1) {
                        ...
                        picklistIdArray.push(pickListId);
                        setImmediate(callbackDone);
                    } else {
    ...
    flowController.on('7', function (updatedArray, resultArray) {
        ...
        if (updatedArray.length == 0) {
            ...
            requestify.post(requestifyUrl, {'pickListArray': updatedArray, pickSubListCount: resultArray, 'deviceId': deviceId, 'userId': userId}).then(function (response) {
...
//
//
pickListService.assignNextLot = function (dataObject, servicecallback) {
    ...
    flowController.on('0.0', function () {
        ...
        function_pickListService.getTotalPendingPickSubListByDeviceId(deviceId, function (err, pickListRecord) {
            ...
            if (err) {
                ...
                pendingPickListCount = pickListRecord;
